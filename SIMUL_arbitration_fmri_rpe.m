...
diary_name = [pwd '/result_save/command_output_' session_opt num2str(SESS_NUMBER) '.txt'];
diary diary.txt
diary on
...
while 1 
    ...
    if keyIsDown
        [tmp, tmp_key_code]=find(keyCode==1);
        if ismember(trigger_key,tmp_key_code) % trigger
%         if keyIsDown
                     break;
        end
        % If the user holds down a key, KbCheck will report multiple events.
...
%         disp(sprintf('line:509, trial=%d, policy=%d, FS=%d, HL=%f',trial,trial_in_block_condition,single_block_condition(4,trial), map_opt.transition_prob_seed(1)));                
        ...
        while(map_sbj.IsTerminal(state_sbj.state_history(state_sbj.index))==0)
            ...
            disp(['line 579: state - ' num2str(current_state) ',action 1 - state ' num2str(find(map_sbj.action(1,1).connection(current_state,:))) ', action 2 -state ' num2str(find(map_sbj.action(1,2).connection(current_state,:)))])
...
%                 DrawFormattedText(wPtr, '준비', 'center', 'center', COLOR_FIXATION_MARK); % add 'o' mark at the click pt.
            end
            Screen(wPtr, 'Flip');
...
%                 [secs, keyCode] = KbPressWait([], clock_time_limit_start+sec_limit_decision); % if no keyboard in time limit, then go ahead. if pressed earlier, then go ahead.
                ...
                [tmp tmp_key_code]=find(keyCode==1);         
                if ismember(KEY_L,tmp_key_code) % L pressed
                    state_sbj.action_history(state_sbj.index)=1;
                    decision_made=1;
                end
                if ismember(KEY_R,tmp_key_code) % R pressed
...
%                 DrawFormattedText(wPtr, unicode_display,'center',ypos2+sy2+60); % add 'o' mark at the click pt.
    %             DrawFormattedText(wPtr, sprintf('Current Session Score: %d \n \nCurrent Session Ranking: Top %.1f%%', current_sess_score,100-current_sess_percent),'center',ypos2+sy2+20); % add 'o' mark at the click pt.
        ...
        disp(sprintf('line:797, arrival=%d, reward=%d, condition=%d state= %d, %d actions=%d, %d', original_rwd,state_sbj.reward_history(end),current_goal_state, state_sbj.state_history(2), state_sbj.state_history(3),state_sbj.action_history(1),state_sbj.action_history(2)));
        Screen(wPtr, 'Flip'); % display on
        WaitSecs(0.5);        
        if(outcome_state~=9)
...
%         DrawFormattedText(wPtr, ['How difficult(0~10)? ' sprintf('%d',conf)], 'center', 'center');
        ...
        if ismember(KEY_R,ans_key_conf)
                if conf<10
                    conf=conf+1;
                end
        elseif ismember(KEY_L,ans_key_conf)
                if conf>0
                    conf=conf-1;
                end
